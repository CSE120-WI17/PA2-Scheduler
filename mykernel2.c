...
void AllocateRemainCPU()
{
	...
	int equal_cpu_util;
	if (num_procs - num_reqs == 0)
		equal_cpu_util = 0;
	else
		equal_cpu_util = (CPUMAX - cpu_alloc)/(num_procs - num_reqs);
	...
	int n;				// percent of CPU time
{
	...
	for (i = 0; i < MAXPROCS; i++){
		...
		if (proctab[i].valid && proctab[i].pid == pid){
            ...
            proctab[i].cpu_portion = cpu_util;
